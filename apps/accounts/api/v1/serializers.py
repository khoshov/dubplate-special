...
class UserUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_phone(self, value):
        ...
        if value and hasattr(self, "instance") and self.instance:
            existing_user = (
                User.objects.filter(phone=value).exclude(pk=self.instance.pk).first()
            )
            if existing_user:
                raise serializers.ValidationError(
...
class UniversalLoginSerializer(serializers.Serializer):
    ...
    password = serializers.CharField(
        required=False, help_text="Пароль (для входа по email)"
    )
    ...
    def validate(self, attrs):
        ...
        if is_email:
            ...
            if not password:
                raise serializers.ValidationError(
                    "Для входа по email необходимо указать пароль"
                )
            ...
            if not code:
                raise serializers.ValidationError(
                    "Для входа по телефону необходимо указать SMS код"
                )

            # Проверяем SMS код
            try:
                verification = SMSVerification.objects.filter(
                    phone_number=identifier, is_verified=False
                ).latest("created_at")
            except SMSVerification.DoesNotExist:
                raise serializers.ValidationError(
                    "SMS код не найден или уже использован"
                )
...
class SendSMSSerializer(serializers.Serializer):
    phone_number = serializers.CharField(
        max_length=20, help_text="Российский номер телефона в любом формате"
    )
    ...
    def save(self):
        ...
        try:
            ...
            if not existing_verification.can_resend():
                raise serializers.ValidationError(
                    "SMS уже отправлена. Повторить можно через 1 минуту"
                )
        except SMSVerification.DoesNotExist:
        ...
        success, message = sms_service.send_verification_code(
            phone_number, verification.code
        )
...
class ResendSMSSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Нет активной верификации для повторной отправки"
            )

        # Проверяем, можно ли отправить повторно
        if not verification.can_resend():
            raise serializers.ValidationError(
                "Повторную отправку можно сделать через 1 минуту"
            )
    ...
    def save(self):
        ...
        return {"message": message, "phone_number": phone_number, "expires_in": 300}
