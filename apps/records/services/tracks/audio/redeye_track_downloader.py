...
def download_to_filefield(
    ...
    *,
    ...
    if track.audio_preview and track.audio_preview.name and not overwrite:
        logger.info(
            "download_to_filefield: already has preview (skip) track=%s", track.pk
        )
    ...
    try:
        ...
        if ct and ct not in ALLOWED_CT and not url.lower().endswith((".mp3", ".aac")):
            logger.warning(
                "download_to_filefield: unexpected content-type %s for %s", ct, url
            )
    ...
    try:
        ...
        logger.info(
            "download_to_filefield: saved %s -> %s", url, track.audio_preview.name
        )
        return track.audio_preview.name
    except Exception as e:
        logger.error(
            "download_to_filefield: save failed for %s: %s", url, e, exc_info=True
        )
        return None
    finally:
        try:
